...
def realizarVenta(prom, Can, Tip):
    if Tip == 1:
        juice.venjNatu(Can + prom)
    elif Tip == 2:
        juice.venjno_Natu(Can + prom)
...
def venta(acc, tip, qui, v):
    if acc == 1:
        ...
        hor = now.time()
        if juice.verifica(can, tip) == 1:
            comp = vent_distacia(can, tip, met, dir, hor)
            info = comp.compra()
            return info
        elif juice.verifica(can, tip) == 0:
            print(f"Error cantidades insuficientes verifique disponibilidad o reabasteca desde la bodega...")
