...
def run_bbot(email, pwd, active, expiration_time=5, money=2, bb_std=2.1, bb_window=20, ema_window=100,
             acc_type='PRACTICE', bot_token=None, bot_chatID=None):
                ...
                if curr_price > bb_hi and curr_ema > bb_hi:
                    ...
                    if mpm[0] == 'turbo':
                        threading.Thread(target=buy_turbo,
                                         args=(iqoapi, active, money, signal, get_expiration_time())).start()
                    elif mpm[0] == 'digital':
                        threading.Thread(target=buy_digital,
                                         args=(iqoapi, active, money, signal, expiration_time)).start()

                    back_to_bb = False
                    timer_trade = Timer(remaining_seconds(), lambda: None)
                    timer_trade.start()

                    text = "IQBBot2.2 Ativo: {} - {}\nTaxa: {}\nSinal: {}\nHorário: {}\nExpiração: {}".format(active,
                                                                                                              mpm[0],
                                                                                                              curr_price,
                                                                                                              signal,
                    ...
                    if bot_token is not None and bot_chatID is not None: telegram_bot_sendtext(bot_token, bot_chatID,
                                                                                               text)
...
if __name__ == '__main__':
    ...
    actives = {expiration: (
        'EURAUD', 'EURAUD-OTC', 'EURCAD', 'EURCHF', 'EURGBP', 'EURJPY', 'EURNZD', 'EURUSD', 'AUDCHF', 'AUDJPY', 'AUDNZD', 'AUDUSD',
    ...
    email = 'yordantarazona23@gmail.com'
    ...
    for expiration_time, active_list in actives.items():
        for active in active_list:
            Process(target=run_bbot, args=(
            email, pwd, active, expiration, money, bb_std, bb_window, ema_window, acc_type, bot_token,
            bot_chatID)).start()
