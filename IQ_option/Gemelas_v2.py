...
def Operacion(dir, lucro, cont, auxvalor):
    if dir:
        print(f"Direccion --> {dir}{' 📈' if dir == 'CALL' else ' 📉'}")

        valor_entrada = auxvalor if cont == 1 else valor_entrada_b
...
while True:
    ...
    #entrar1 = True if (minutos >= 0.59 and minutos <= 0.59) or (minutos >= 5.59 and minutos <= 5.59) else False
    entrar2 = True if (minutos >= 3.59 and minutos <= 3.59) or (minutos >= 8.59 and minutos <= 8.59) else False
    """
    if entrar1 == True:
        print(f"\n\nHora -- > {hora}")
        dir, lucro = Estrategia(lucro)
        Operacion(dir, lucro, cont1, auxvalor1)
        """
    if entrar2 == True:
        print(f"\n\nHora -- > {hora}")
        dir, lucro = Estrategia(lucro)
        Operacion(dir, lucro, cont2, auxvalor2)
