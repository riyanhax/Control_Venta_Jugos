...
''' This Bot Checks if the first prediction was right before entering a new position '''
...
def preview():
    ...
    rand_frst_clf = RandomForestClassifier(
        n_estimators=400, oob_score=True, criterion="entropy", random_state=0)
    ...
    d = {'From': [teste+timedelta(seconds=60*i) for i in range(len(y_pred))], 'Pred': [
        ...
        while True:
            ...
            if tempo_servidor + timedelta(seconds=2) == sinal:
                ...
                ult_vela = API.get_candles(
                    par, 60, 1, API.get_server_timestamp())
                ...
                if ult_vela == df['Pred'].iloc[0]:
                    ...
                    print('Modelo Errou Primeiro Palpite')
                    erro = True
