    ...
    if direcao:
        ...
        while martingale_count <= QTD_MARTINGALE:
            if delay_aceitavel():
                ...
                if status:
                    valor_resultante, lucro, wins = check_result(id, entrada, lucro, martingale_count, mhi_count,
                                                                 delay_compra, wins)
...
def printar_resultado(hit, entrada, valor_resultante, lucro, martingale_count, delay_compra):
    ...
    if payout:
        ...
        print(str_result, round(valor_resultante, 2), '/ PAYOUT:', str(round(payout, 2)) + '%', '/ LUCRO:',
              round(lucro, 2))
...
for paridade in par1['digital']:
    if par1['digital'][paridade]['open'] == True:
        print('[ DIGITAL ]: ' + paridade + ' | Payout: ' + str(payout(paridade)))
        """
